...
def _reg_l1_loss(outputs, mask, ind, target):
    #print(output.shape)
    loss=0.
    for output in outputs:
        pred = _tranpose_and_gather_feature(output, ind)
        mask = mask.float()
        # loss = F.l1_loss(pred * mask, target * mask, reduction='elementwise_mean')
        loss_temp = F.l1_loss(pred * mask, target * mask, size_average=False)
        loss += loss_temp / (mask.sum() + 1e-4)
    return loss