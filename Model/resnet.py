...
class PoseResNet(nn.Module):
    def __init__(self, block, layers, head_conv, num_classes, num_joints):
        ...
        if head_conv > 0:
            ...
            # 关键点的heatmap
            self.hm_hp = nn.Sequential(nn.Conv2d(256, head_conv, kernel_size=3, padding=1),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(head_conv, num_joints, kernel_size=1))
            ...
            self.w_h_ = nn.Sequential(nn.Conv2d(256, head_conv, kernel_size=3, padding=1),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(head_conv, 2, kernel_size=1))
            self.hps = nn.Sequential(nn.Conv2d(256, head_conv, kernel_size=3, padding=1),
                                        nn.ReLU(inplace=True),
                                        nn.Conv2d(head_conv, 2*num_joints, kernel_size=1))
        else:
            # heatmap layers
            self.hmap = nn.Conv2d(in_channels=256, out_channels=num_classes, kernel_size=1)
            self.hm_hp = nn.Conv2d(in_channels=256, out_channels=num_joints, kernel_size=1)
            # regression layers
            ...
            self.hps = nn.Conv2d(in_channels=256, out_channels=2*num_joints, kernel_size=1)
    ...
    def forward(self, x):
        ...
        out = [[self.hmap(x), self.regs(x), self.w_h_(x), self.hm_hp(x), self.hps(x)]]
        #out = [[self.hmap(x), self.regs(x), self.w_h_(x), self.hm_hp(x)]]
        return out
...
def get_pose_net(num_layers, head_conv, num_joints, num_classes=1):
    block_class, layers = resnet_spec[num_layers]

    model = PoseResNet(block_class, layers, head_conv=head_conv, num_classes=num_classes, num_joints = num_joints)
...
if __name__ == '__main__':
    ...
    net = get_pose_net(num_layers=18, head_conv=64, num_joints=12)
